...
/**
    ...
    **It reallocates only when it has to.** `Ensure()` is called every frame and
    is a no-op in the overwhelming majority of them. Astronaught's buffers are sized
    ...
    **It actually frees its colour texture.** `ffglex::FFGLFBO::Release()`
    ...
    Astronaught is safe by construction -- `ProcessOpenGL` calls every `Ensure()`
    ...
    Nothing in Astronaught samples a reduced level: the compander's high-pass is a
